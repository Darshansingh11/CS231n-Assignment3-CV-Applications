...
def content_loss(content_weight, content_current, content_original):
    ...
    content_current = torch.squeeze(content_current)
    content_current = content_current.view(content_current.size()[0], -1)

    content_original = torch.squeeze(content_original)
    content_original = content_original.view(content_original.size()[0], -1)

    return (content_weight * (torch.norm(content_current - content_original) ** 2))
...
def gram_matrix(features, normalize=True):
    ...
    N, C, H, W = features.size()[0], features.size()[1], features.size()[2], features.size()[3]
    features = features.view(N, C, -1)
    gram = features[0, :, :].mm(features[0, :, :].t())
    for i in range(1, N):
      gram = torch.cat((gram, features[i, :, :].mm(features[i, :, :].t())), 0)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for (i, layer) in enumerate(style_layers):
      current_image_gm = gram_matrix(feats[layer])
      style_loss += style_weights[i] * ((torch.norm(current_image_gm - style_targets[i])) ** 2)
...
def tv_loss(img, tv_weight):
    ...
    vertical_shifted = img[:, 1:, :]
    vertical_shifted = torch.cat((vertical_shifted, img[:, H - 1:H, :]), 1)

    horizontal_shifted = img[:, :, 1:]
    horizontal_shifted = torch.cat((horizontal_shifted, img[:, :, W - 1:W]), 2)

    loss += ((torch.norm(img - vertical_shifted)) ** 2)
    loss += ((torch.norm(img - horizontal_shifted)) ** 2)
