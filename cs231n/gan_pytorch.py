...
def sample_noise(batch_size, dim, seed=None):
    ...
    return -1 + (torch.rand(batch_size, dim) * 2)
...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss_r = bce_loss(logits_real, torch.ones_like(logits_real))
    loss_f = bce_loss(logits_fake, torch.zeros_like(logits_fake))
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss_r = torch.mean((scores_real - 1) ** 2)
    loss_f = torch.mean(scores_fake ** 2)
